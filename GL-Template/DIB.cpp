...
CDib::CDib(CBitmap& bitmap)
{
	...
	*lpbi = bi;
	...
	m_pBits = (LPBYTE)lpbi + (bi.biSize + nColors * sizeof(RGBQUAD));
...
BOOL CDib::Paint(HDC hDC, CRect rcDC, CRect rcDIB)
{
	...
	BOOL bSuccess;
	if(rcDC.Width() == rcDIB.Width() && rcDC.Height() == rcDIB.Height()) 
	{
		bSuccess = ::SetDIBitsToDevice(hDC,							// hDC
									rcDC.left,						// DestX
									rcDC.top,						// DestY
									rcDC.Width(),					// nDestWidth
									rcDC.Height(),					// nDestHeight
									rcDIB.left,						// SrcX
									(int)Height() - rcDIB.bottom,	// SrcY
									0,								// nStartScan
...
{
	CFile file;
	file.Open((LPCTSTR)filename,CFile::modeCreate | CFile::modeWrite | CFile::shareDenyNone);
	DWORD rez=Save(file);
	file.Close();
	return rez;
}
...
DWORD CDib::Read(char* filename)
{
	CFile file;
	file.Open((LPCTSTR)filename,CFile::modeRead | CFile::shareDenyNone);
	DWORD rez=Read(file);
	file.Close();
	return rez;
}
...
#ifdef _DEBUG
void CDib::Dump(CDumpContext& dc)
{
	CObject::Dump(dc);
}
#endif
